...
const CloudRecruit = ({ showToast, userInfo }) => {
  ...
  const fetchJobDescription = async () => {
    ...
    setLoading(true);
    try {
      const fileName = `${selectedLevel}_${selectedCloudProvider}_Cloud.txt`;
  ...
  const uploadResume = async () => {
    ...
    // Prepare data for upload status tracking
    const initialData = {
      hrEmail,
      rrfId,
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ color: "black", margin: 0 }}>Cloud Recruitment</h1>
