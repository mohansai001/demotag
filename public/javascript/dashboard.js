...
function updateNewChartData(teamACount, teamBCount, teamCCount, teamDCount) {
  newStatusChart.data.datasets[0].data = [
    teamACount,
    teamBCount,
    teamCCount,
    teamDCount,
...
function updateNewBarChartData(teamACount, teamBCount, teamCCount, teamDCount) {
  newRoleChart.data.datasets[0].data = [
    teamACount,
    teamBCount,
    teamCCount,
    teamDCount,
...
async function fetchNewCountsAndUpdateChart() {
  try {
    ...
    const teamBCount = await fetch(
      ...
      .then((res) => res.json())
      .then((data) => data.count || 0);
    const teamCCount = await fetch(
      "https://demotag.vercel.app/api/hadoop-resume-count"
    )
      .then((res) => res.json())
      .then((data) => data.count || 0);
    const teamDCount = await fetch(
      "https://demotag.vercel.app/api/java-resume-count"
    )
      .then((res) => res.json())
      .then((data) => data.count || 0);
...
const secondStatusCtx = document
  .getElementById("secondStatusChart")
  .getContext("2d");
const secondRoleCtx = document
  .getElementById("secondRoleChart")
  .getContext("2d");
...
function updateSecondApplicantCountsHTML(
  ...
  ).innerText = `${dataEngineerCount} Applicants`;
  document.getElementById(
    "data-ops-engineer-applicants"
  ).innerText = `${dataOpsEngineerCount} Applicants`;
  document.getElementById(
    "data-bi-visualization-applicants"
  ).innerText = `${dataBIVisualizationCount} Applicants`;
  document.getElementById(
    "data-modeller-applicants"
  ).innerText = `${dataModellerCount} Applicants`;
  document.getElementById(
    "data-analyst-applicants"
  ).innerText = `${dataAnalystCount} Applicants`;
  document.getElementById(
    "data-architect-applicants"
  ).innerText = `${dataArchitectCount} Applicants`;
  document.getElementById(
    "data-scientist-applicants"
  ).innerText = `${dataScientistCount} Applicants`;
}
...
async function fetchAndUpdateSecondCharts() {
  try {
    const [
      ...
      fetchNewCounts(
        "https://demotag.vercel.app/api/data-resume-count",
        "Data Engineer"
      ),
      fetchNewCounts(
        "https://demotag.vercel.app/api/data-ops-resume-count",
        "Data-Ops Engineer"
      ),
      fetchNewCounts(
        "https://demotag.vercel.app/api/data-bi-resume-count",
        "Data – BI Visualization Engineer"
      ),
      fetchNewCounts(
        "https://demotag.vercel.app/api/data-modeller-resume-count",
        "Data Modeller"
      ),
      fetchNewCounts(
        "https://demotag.vercel.app/api/data-analyst-resume-count",
        "Data Analyst"
      ),
      fetchNewCounts(
        "https://demotag.vercel.app/api/data-architect-resume-count",
        "Data Architect"
      ),
      fetchNewCounts(
        "https://demotag.vercel.app/api/data-scientist-resume-count",
        "Data Scientist – AI/ML"
      ),
    ]);
...
async function fetchTotalCountAndUpdateCharts(ecMapping) {
  try {
    ...
    const response = await fetch(
      `https://demotag.vercel.app/api/candidate-total-by-team?ecs=${encodeURIComponent(
        ecs
      )}`
    );
    ...
    const departmentRoleCtx = document
      .getElementById("departmentRoleChart")
      .getContext("2d");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (ecMappingParam) {
    ...
    history.replaceState(
      null,
      "",
      `?ec_mapping=${encodeURIComponent(ecMappingParam)}`
    );
  } else {
    ...
    if (storedEcMapping) {
      ...
      history.replaceState(
        null,
        "",
        `?ec_mapping=${encodeURIComponent(storedEcMapping)}`
      );
    }
  }
  ...
  if (ecMappingParam) {
    ...
    ecMapping.forEach((ec) => {
      switch (ec) {
        case "Cloud EC":
          document
            .querySelectorAll(".dashboard-charts .dashboard-chart")
            .forEach((chart) => {
        ...
        case "App EC":
          document
            .querySelectorAll(".dashboard-charts .dashboard-chart1")
            .forEach((chart) => {
              const h3 = chart.querySelector("h3");
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    const totalApplicants = Object.values(aggregatedData).reduce(
...
async function fetchDevOpsCount() {
  try {
    const response = await fetch(
      "https://demotag.vercel.app/api/devops-resume-count"
    );
...
async function fetchplatformCount() {
  try {
    const response = await fetch(
      "https://demotag.vercel.app/api/platform-resume-count"
    );
    if (!response.ok) throw new Error("Network response was not ok");
    const data = await response.json();
...
async function fetchcloudopsCount() {
  try {
    const response = await fetch(
      "https://demotag.vercel.app/api/cloudops-resume-count"
    );
    if (!response.ok) throw new Error("Network response was not ok");
    const data = await response.json();
...
async function updateApplicantCounts() {
  try {
    ...
    document.getElementById(
      "devops-applicants"
    ).innerText = `${devOpsCount} Applicants`;
    document.getElementById(
      "platform-applicants"
    ).innerText = `${platformCount} Applicants`;
    document.getElementById(
      "cloudops-applicants"
    ).innerText = `${cloudOpsCount} Applicants`;
    document.getElementById(
      "site-applicants"
    ).innerText = `${siteCount} Applicants`;
    document.getElementById(
      "data-engineer-applicants"
    ).innerText = `${dataEngineer}`;
    document.getElementById(
      "bi-visualization-applicants"
    ).innerText = `${dataBIVisualizationCount}`;
    document.getElementById(
      "data-analyst-applicants"
    ).innerText = `${dataAnalystCount}`;
    document.getElementById(
      "data-modeller-applicants" 
    ).innerText = `${dataModellerCount}`;
    document.getElementById(
      "cloud-native-backend-applicants"
    ).innerText = `${backendCount}`;
  } catch (error) {
...
async function loadCandidateCounts(ec_mapping) {
  ...
  try {
    ...
    const response = await fetch(
      `https://demotag.vercel.app/api/candidate-counts?eng_center=${encodeURIComponent(
        ec_mapping
      )}`
    );
    const data = await response.json();
...
async function updateChartAndMetrics() {
  ...
  const numbers = {
    totalApplicants:
      counts.cloneJavaBackend + counts.dataPythonPySpark + counts.combinedCloud,
  ...
  const appNumber = document.querySelector(
    ".metrics-grid .dot-job-boards + div .number"
  );
  const cloudNumber = document.querySelector(
    ".metrics-grid .dot-referrals + div .number"
  );
  const dataNumber = document.querySelector(
    ".metrics-grid .dot-social + div .number"
  );
  const coreNumber = document.querySelector(
    ".metrics-grid .dot-agencies + div .number"
  );
  ...
  let gradientString = [
    ...
    `#7de7d0 ${angleStart + (appPercent / 100) * 360}deg ${
      angleStart + ((appPercent + cloudPercent) / 100) * 360
    }deg`, // Cloud (combined count) color
    `#c3f4ff ${angleStart + ((appPercent + cloudPercent) / 100) * 360}deg ${
      angleStart + ((appPercent + cloudPercent + dataPercent) / 100) * 360
    }deg`, // Data (Data_EC_ERIE_Python_PySpark) color
    `#b4ecd7 ${
      angleStart + ((appPercent + cloudPercent + dataPercent) / 100) * 360
    }deg ${360}deg`, // Core color
  ].join(", ");
...
function attachFilters() {
  ...
  document
    .getElementById("rejectedDateFilter")
    .addEventListener("input", function () {
...
function filterTable(columnIndex, filterValue) {
  ...
  rows.forEach((row) => {
    ...
    if (cell) {
      ...
      row.style.display = cellText.toLowerCase().includes(filterValue)
        ? ""
        : "none";
    }
  });
...
document
  .getElementById("shortlistedCard")
...
function attachFiltersToShortlistedTable() {
  ...
  document
    .getElementById("shortlistedNameFilter")
    .addEventListener("input", function () {
      filterShortlistedTable(1, this.value);
    });

  document
    .getElementById("shortlistedEmailFilter")
    .addEventListener("input", function () {
      filterShortlistedTable(2, this.value);
    });

  document
    .getElementById("shortlistedStatusFilter")
    .addEventListener("input", function () {
      filterShortlistedTable(3, this.value);
    });

  document
    .getElementById("shortlistedRoleFilter")
    .addEventListener("input", function () {
      filterShortlistedTable(4, this.value);
    });

  document
    .getElementById("shortlistedPhaseFilter")
    .addEventListener("input", function () {
      filterShortlistedTable(5, this.value);
    });

  document
    .getElementById("shortlistedReasonFilter")
    .addEventListener("input", function () {
...
function attachFiltersToRejectedTable() {
  ...
  document
    .getElementById("rejectedNameFilter")
    .addEventListener("input", function () {
      filterRejectedTable(1, this.value);
    });

  document
    .getElementById("rejectedEmailFilter")
    .addEventListener("input", function () {
      filterRejectedTable(2, this.value);
    });

  document
    .getElementById("rejectedStatusFilter")
    .addEventListener("input", function () {
      filterRejectedTable(3, this.value);
    });

  document
    .getElementById("rejectedRoleFilter")
    .addEventListener("input", function () {
      filterRejectedTable(4, this.value);
    });

  document
    .getElementById("rejectedPhaseFilter")
    .addEventListener("input", function () {
      filterRejectedTable(5, this.value);
    });

  document
    .getElementById("rejectedReasonFilter")
    .addEventListener("input", function () {
...
async function fetchPrescreeningCount() {
  try {
    ...
    const response = await fetch(
      "https://demotag.vercel.app/api/prescreening-count"
    );
    const data = await response.json();
  ...
  let hrChart = new Chart(ctx, {
    ...
    data: {
      labels: [
        ...
        "Client Interview",
        "Shortlisted",
      ], // All recruitment phases as labels
...
function toggleDateInputs() {
  ...
  customDateRange.style.display =
    filterType === "custom_range" ? "block" : "none";
}
...
async function fetchImocha() {
  try {
    ...
    const shortlistedRes = await fetch(
      "https://demotag.vercel.app/api/shortlisted-count"
    );
    const shortlistedData = await shortlistedRes.json();
    document.getElementById("inviteCount").innerText =
      shortlistedData.inviteCount;

    // Fetch Available Score Count
    const scoreRes = await fetch(
      "https://demotag.vercel.app/api/available-score-count"
    );
    const scoreData = await scoreRes.json();
    document.getElementById("availableScoreCount").innerText =
      scoreData.availableScoreCount;
  } catch (error) {
